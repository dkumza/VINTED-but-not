   ...
   return (
      <div className="new">
         <div className="w-full flex relative">
            {cats && (
               <div className="book-id bg-yellow-400 text-sm font-semibold text-white shadow py-1 px-2 absolute -top-5 -right-4 h-8">
                  {cats.find((c) => c.id === vint.cat)?.title}
               </div>
            )}

            {users && (
               <div className="book-id bg-green-500 text-sm font-semibold text-white shadow py-1 px-2 absolute -top-5 -left-4 flex h-8 gap-2 items-center">
                  {(() => {
